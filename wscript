...
def configure(conf):
    ...
    if not auto_configured:
        if not conf.check_cxx(lib='zip', header_name='zip.h',
            ...
            conf.fatal("\n\n  Cannot find libzip, required for zipper,\n  please install from:\n  'http://nih.at/libzip/'\n  (see README.md for more info)\n")
        else:
...
def build(bld):
    ...
    obj.target = TARGET
    obj.source = "src/_zipper.cc"
    obj.source += " src/zipper.cpp"
    obj.uselib = "ZIPFILE"
    start_dir = bld.path.find_dir('lib')
    # http://www.freehackers.org/~tnagy/wafbook/index.html#_installing_files
    bld.install_files('${LIBPATH_NODE}/node/zipper', start_dir.ant_glob('*'), cwd=start_dir, relative_trick=True)
